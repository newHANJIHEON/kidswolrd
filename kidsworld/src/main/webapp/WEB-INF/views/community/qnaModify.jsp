...
<div id="qnawrite_wrapwrap">
    <sec:authorize access="isAuthenticated()">
        ...
        <c:choose>
            <c:when test="${loginMember.uNum == qna.qUserNum}">
                <form action="${path}/community/qnaModify" method="post">
                    <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                    <input type="hidden" name="qNum" value="${qna.qNum}" />
                    <input type="hidden" name="uNum" value="${loginMember.uNum}">
                    <input type="hidden" name="qUserNum" value="${qna.qUserNum}" />
                    <table id="qnawrite_wrap">
                        <tr>
                            <td class="box1 title_box">제목</td>
                            <td class="box2 title_box">
                                <input class="subbox1" type="text" name="qTitle" value="${qna.qTitle}" />
                            </td>
                        </tr>
                        <tr>
                            <td class="box1">작성자</td>
                            <td class="box2">
                                <input class="subbox1" type="text" name="qWriter" value="${qna.qWriter}" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="box1">내용</td>
