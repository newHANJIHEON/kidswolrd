...
<div id="notice_wrapwrap">
    <h1 id="notice_headtitle">공지사항</h1>
    <table id="notice_wrap">
        <tr>
            <td class="box1 title_box">카 테 고 리</td>
                ...
                <c:forEach var="notices" items="${list}">
                    <c:choose>
                        <c:when test="${notices.nDelete eq 'N'}">
                            <tr>
                                <td class="box1">${notices.nCategory}</td>
                                <td class="box2" id="noticeText_box">
                                    <a href="${path}/community/noticeRead?nNum=${notices.nNum}&page=${pm.cri.page}&perPageNum=${pm.cri.perPageNum}">${notices.nTitle}</a>
                                </td>
                                <td class="box3">${notices.nWriter}</td>
                                <td class="box4"><f:formatDate value="${notices.nDate}" pattern="yyyy-MM-dd" /></td>
                            </tr>
                        </c:when>
                    </c:choose>
                </c:forEach>
                <c:if test="${!empty pm}">
                    <tr>
                        <td class="nextbox" colspan="4">
                            <c:if test="${pm.prev}">
                                <a href="${path}/community/notice${pm.search(pm.startPage - 1)}">[이전]</a>
                            </c:if>
                            <c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <a href="${path}/community/notice${pm.search(i)}">[${i}]</a>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                            <c:if test="${pm.next}">
                                <a href="${path}/community/notice${pm.search(pm.endPage + 1)}">[다음]</a>
                            </c:if>
                        </td>
                    </tr>
                </c:if>
    ...
    <sec:authorize access="hasRole('ROLE_MASTER')">
        <a id="noticeBtn" href="${path}/community/noticeWrite">공지사항 쓰기</a>
    </sec:authorize>
</div>
