...
<div id="manager_wrapwrap">
    <h1 id="manager_headtitle">되돌리기</h1>
    <sec:authorize access="hasRole('ROLE_MASTER')">
        <form action="${path}/master/usedTicketInfo" method="get">
        ...
        <form action="${path}/master/rollbackTicket" method="post">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            <table id="manager_wrap">
                <tr>
                    ...
                    <td class="box9 title_box">사용일</td>
                    <td class="box10 title_box">비사용체크</td>
                </tr>
                <c:choose>
                    <c:when test="${!empty list}">
                        <c:forEach var="ticket" items="${list}">
                            <tr>
                                ...
                                <td class="box10 textstyle">
                                    <input type="checkbox" name="tNum" value="${ticket.tNum}" />
                                </td>
                            </tr>
                        </c:forEach>
                        <tr>
                            <td colspan="10">
                                <input id="managerBtn" type="submit" value="되돌리기" />
                            </td>
                        </tr>
                        <c:if test="${!empty pm}">
                            <tr>
                                <td class="nextbox" colspan="10">
                                    <c:if test="${pm.prev}">
                                        <a href="${path}/master/usedTicketInfo${pm.search(pm.startPage - 1)}">[이전]</a>
                                    </c:if>
                                    <c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}">
                                        <c:choose>
                                            ...
                                            <c:otherwise>
                                                <a href="${path}/master/usedTicketInfo${pm.search(i)}">[${i}]</a>
                                            </c:otherwise>
                                        </c:choose>
                                    </c:forEach>
                                    <c:if test="${pm.next}">
                                        <a href="${path}/master/usedTicketInfo${pm.search(pm.endPage + 1)}">[다음]</a>
                                    </c:if>
                                </td>
                            </tr>
                        </c:if>
