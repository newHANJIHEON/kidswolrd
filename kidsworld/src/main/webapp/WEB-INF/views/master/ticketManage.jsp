...
<div id="manager_wrapwrap">
    <h1 id="manager_headtitle">사용처리</h1>
    <sec:authorize access="hasRole('ROLE_MASTER')">
    <form action="${path}/master/ticketManage" method="get">
    ...
    <form action="${path}/master/uesTicket" method="post">
        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
        <table id="manager_wrap">
            <tr>
                ...
                <td class="box9 title_box">사용체크</td>
            </tr>
            <c:choose>
                <c:when test="${!empty list}">
                    ...
                    <tr>
                        <td colspan="9">
                            <input id="managerBtn" type="submit" value="사용처리" />
                        </td>
                    </tr>
                    <c:if test="${!empty pm}">
                        <tr>
                            <td class="nextbox" colspan="9">
                                <c:if test="${pm.prev}">
                                    <a href="${path}/master/ticketManage${pm.search(pm.startPage - 1)}">[이전]</a>
                                </c:if>
                                <c:forEach var="i" begin="${pm.startPage}" end="${pm.endPage}">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <a href="${path}/master/ticketManage${pm.search(i)}">[${i}]</a>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                                <c:if test="${pm.next}">
                                    <a href="${path}/master/ticketManage${pm.search(pm.endPage + 1)}">[다음]</a>
                                </c:if>
                            </td>
                        </tr>
                    </c:if>
